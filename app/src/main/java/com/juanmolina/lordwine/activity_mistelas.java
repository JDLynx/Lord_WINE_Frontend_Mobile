...
public class activity_mistelas extends AppCompatActivity {

    private RecyclerView recyclerViewMistelas;
    private ProgressBar progressBarLoading;
    private TextView textViewStatusMessage;
    private ProductosAdapter productosAdapter;
    private List<Producto> listaMistelas;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mistelas);

        recyclerViewMistelas = findViewById(R.id.recyclerViewMistelas);
        progressBarLoading = findViewById(R.id.progressBarLoading);
        textViewStatusMessage = findViewById(R.id.textViewStatusMessage);

        bottomNavigationView = findViewById(R.id.bottom_navigation_mistelas);
        drawerLayout = findViewById(R.id.drawer_layout_mistelas);
        navigationView = findViewById(R.id.nav_view_mistelas);

        categoriesList = new ArrayList<>();
        allProductsList = new ArrayList<>();
        listaMistelas = new ArrayList<>();
        productosAdapter = new ProductosAdapter(this, listaMistelas);

        recyclerViewMistelas.setLayoutManager(new GridLayoutManager(this, 2));
        recyclerViewMistelas.setAdapter(productosAdapter);
    ...
    private void fetchProductos() {
        ...
        recyclerViewMistelas.setVisibility(View.GONE);
        ...
        call.enqueue(new Callback<List<Producto>>() {
            ...
            public void onResponse(Call<List<Producto>> call, Response<List<Producto>> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Log.d("API_CALL", "Productos cargados de la API: " + allProductsList.size());
                    filterAndShowProducts(allProductsList, "Mistela");
                } else {
                    Log.e("API_CALL", "Error al cargar productos. Código de respuesta: " + response.code());
    ...
    private void filterAndShowProducts(List<Producto> productosData, String categoryName) {
        ...
        Log.d("FilterDebug", "Filtrando por '" + categoryName + "'. Productos encontrados: " + productosFiltrados.size());
        ...
        if (productosFiltrados.isEmpty()) {
            ...
            listaMistelas.clear();
            listaMistelas.addAll(productosFiltrados);
            productosAdapter.notifyDataSetChanged();
            recyclerViewMistelas.setVisibility(View.VISIBLE);
            textViewStatusMessage.setVisibility(View.GONE);
        }
    }
        ...
        call.enqueue(new Callback<List<Category>>() {
            ...
            public void onResponse(Call<List<Category>> call, Response<List<Category>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Log.d("CategoryFetch", "Categorías cargadas: " + categoriesList.size());
                    for (Category category : categoriesList) {
                        navigationView.getMenu().add(0, category.getId(), 0, category.getName());
                        Log.d("CategoryFetch", "Añadida categoría al menú: " + category.getName() + " (ID: " + category.getId() + ")");
                    }
                } else {
                    Log.e("CategoryFetch", "Error al obtener categorías. Código: " + response.code());
                }
            }
            @Override
            public void onFailure(Call<List<Category>> call, Throwable t) {
                Log.e("CategoryFetch", "Fallo al obtener categorías: " + t.getMessage());
            }
        });
    }

    private void fetchMistelas() {
        fetchProductos();
    }
